...
export const handler = async (
  ...
  try {
    ...
    const rows = await query(`
      INSERT INTO profiles (full_name, title, tagline, bio, photo_url, linkedin_url, github_url, email, hero_stats)
      ...
      ON CONFLICT (id) DO UPDATE SET
        ...
        updated_at   = now()
      RETURNING *`,
      [full_name, title, tagline, bio, photo_url, linkedin_url, github_url, email, JSON.stringify(hero_stats ?? [])]
    );
